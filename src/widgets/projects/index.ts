...
import { ProjectsWidgetApp, ProjectsWidgetConfig } from "./ProjectsWidgetApp";
import { ensureWidgetFonts } from "../common";
import tailwindStyles from "@/app/globals.css?inline";

class PayrollProjectsElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this.mountPoint) {
      ...
      this.mountPoint.id = "payroll-projects-mount";
  ...
  private renderApp() {
    ...
    const config: ProjectsWidgetConfig = {
    ...
    this.root.render(React.createElement(ProjectsWidgetApp, { config }));
  }
...
if (typeof window !== "undefined" && !customElements.get("payroll-projects")) {
  customElements.define("payroll-projects", PayrollProjectsElement);
}

export { PayrollProjectsElement };
