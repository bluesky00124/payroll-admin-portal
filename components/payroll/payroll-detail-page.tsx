...
function EmployeeBankTab({ workspace, lines, query, onQueryChange, canViewSensitive }: { workspace: PayrollWorkspace; lines: PayrollLine[]; query: string; onQueryChange: (value: string) => void; canViewSensitive: boolean }) {
  return <section className="payroll-detail-section"><SectionToolbar title="Nhân sự & thông tin ngân hàng" description="Thông tin định danh và chi trả tại thời điểm lập bảng lương." query={query} onQueryChange={onQueryChange} /><div className="payroll-sensitive-note"><ShieldCheck /><span>{canViewSensitive ? "Bạn đang xem với quyền Kế toán; dữ liệu định danh được hiển thị đầy đủ." : "CCCD và số tài khoản được che theo vai trò hiện tại."}</span></div><div className="payroll-table-wrap payroll-bank-table-wrap"><table className="payroll-table payroll-bank-table"><thead><tr><th>Người lao động</th><th className="text-center">CCCD</th><th className="text-center">Điện thoại</th><th className="text-center">Ngày vào làm</th><th>Bộ phận</th><th>Chức vụ</th><th className="text-center">Số tài khoản</th><th>Ngân hàng</th><th className="text-center">Hình thức</th><th className="text-right">Thực lãnh</th></tr></thead><tbody>{lines.map((line) => { const employee = workspace.employees.find((item) => item.id === line.employeeId); const detail = getPayrollLineDetail(line); return <tr key={line.id}><td><div className="line-employee"><UserAvatar name={line.employeeName} size="sm" /><div><strong>{line.employeeName}</strong><small>{line.employeeCode}</small></div></div></td><td className="text-center font-mono">{maskValue(employee?.idCard, canViewSensitive, 3)}</td><td className="text-center font-mono">{maskValue(employee?.phone, canViewSensitive, 3)}</td><td className="text-center">{employee?.joinDate ? formatDate(employee.joinDate) : "—"}</td><td>{employee?.department ?? "—"}</td><td>{line.position}</td><td className="text-center bank-account-cell">{maskValue(detail.payment.bankAccount, canViewSensitive)}</td><td>{detail.payment.bankName}</td><td className="text-center"><Badge tone={detail.payment.method === "transfer" ? "info" : "neutral"}>{detail.payment.method === "transfer" ? "Chuyển khoản" : "Tiền mặt"}</Badge></td><td className="text-right"><strong className="money-value">{formatCurrency(line.netPay)}</strong></td></tr>; })}</tbody></table></div></section>;
}
